...
from person_object_detection.config import (
    CLASSES, RESIZE_TO, TRAIN_DIR, BATCH_SIZE
)
from torch.utils.data import Dataset, DataLoader
from indoor_object_detection.custom_utils import collate_fn, get_train_transform, get_valid_transform

# The dataset class.
class CustomDataset(Dataset):
    print("inside CustomDataset")
    ...
    def __getitem__(self, idx):
        ...
        if torch.isnan(target['boxes']).any().item() or target['boxes'].numel() == 0:
            target['boxes'] = torch.zeros((0, 4), dtype=torch.float32)
            target['labels'] = torch.zeros((0,), dtype=torch.int64)
            target['area'] = torch.zeros((0,), dtype=torch.float32)
            target['iscrowd'] = torch.zeros((0,), dtype=torch.int64)
...
def create_train_dataset(DIR):
    print("inside create_train_dataset")
...
def create_train_loader(train_dataset, num_workers=0):
    print("inside create_train_loader")
